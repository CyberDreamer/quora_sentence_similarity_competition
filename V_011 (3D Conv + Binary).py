...
def PrepareData(data, filename):
    ...
    # X = X.reshape((X.shape[0], 2, X.shape[1], X.shape[2]))
...
def Test(byWeights):
    ...
    ids = np.zeros(2345796, dtype=np.int32)
    ...
    full_sourceData = Load_and_Prefiltred_Data('test.csv', 0, 2345796)
    ...
    for m in range(ss):
        ...
        ids[start:end] = full_sourceData['test_id'].iloc[start:end]
    ...
    SaveResult(y_test, withIndex=False, ids=ids)
...
experimentName = '3D Conv + Binary'
modelName = experimentName + '/model' + '_3D Conv + Binary'  + '.mdl'
weightsName = experimentName + '/w-imp-24-0.3754.hdf5'
...
# lenght = 404290
# lenght = 2345796
# Train(isNew=False, start=0, end=404290)
# CrossTest(byWeights=False, start=300000, end=404290)
Test(byWeights=True)